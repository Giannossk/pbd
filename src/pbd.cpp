...
bool pbd::solve_VolumeConstraint(
	const Vector3r &p0, Real invMass0,		
	const Vector3r &p1, Real invMass1,
	const Vector3r &p2, Real invMass2,
	const Vector3r &p3, Real invMass3,
...
bool pbd::init_IsometricBendingConstraint(
	const Vector3r &p0, 
	const Vector3r &p1, 
	const Vector3r &p2, 
	const Vector3r &p3, 
	Matrix4r &Q)
{
	...
	const Real c01 = matrix::cotTheta(e0, e1);
	const Real c02 = matrix::cotTheta(e0, e2);
	const Real c03 = matrix::cotTheta(-e0, e3);
	const Real c04 = matrix::cotTheta(-e0, e4);
...
bool pbd::solve_IsometricBendingConstraint(
	const Vector3r &p0, Real invMass0, 
	const Vector3r &p1, Real invMass1, 
	const Vector3r &p2, Real invMass2, 
	const Vector3r &p3, Real invMass3, 
...
bool pbd::solve_EdgePointDistanceConstraint(
...
bool pbd::solve_TrianglePointDistanceConstraint(
	const Vector3r &p, Real invMass,
	const Vector3r &p0, Real invMass0,
	const Vector3r &p1, Real invMass1,
	const Vector3r &p2, Real invMass2,
...
bool pbd::solve_EdgeEdgeDistanceConstraint(
	const Vector3r &p0, Real invMass0,
	const Vector3r &p1, Real invMass1,
	const Vector3r &p2, Real invMass2,
	const Vector3r &p3, Real invMass3,
...
bool pbd::init_ShapeMatchingConstraint(
...
bool pbd::solve_ShapeMatchingConstraint(
	...
	Vector3r corr[], Matrix3r *rot)
{
	...
	if (allowStretch)
		...
		//decomposition::polarDecomposition(mat, R, U, D);
		decomposition::polarDecompositionStable(mat, eps, R);
...
bool pbd::init_StrainTetraConstraint(		
	const Vector3r &p0,
	const Vector3r &p1,
	const Vector3r &p2,
	const Vector3r &p3,
...
bool pbd::solve_StrainTetraConstraint(
	const Vector3r &p0, Real invMass0, 
	const Vector3r &p1, Real invMass1,
	const Vector3r &p2, Real invMass2,
	const Vector3r &p3, Real invMass3,
	...
	Vector3r &corr0, Vector3r &corr1, Vector3r &corr2)
{
	...
	const Matrix<Real, 3, 2> piolaKirchhoffStres = F * stress;
	...
	// compute gradient
	Matrix<Real, 3, 2> H = area * piolaKirchhoffStres * invRestMat.transpose();
	Vector3r gradC[3];
...
bool pbd::init_FEMTetraConstraint(			// compute only when rest shape changes
		const Vector3r &p0,
		const Vector3r &p1,
		const Vector3r &p2,
		const Vector3r &p3,
...
void pbd::computeGreenStrainAndPiolaStress(
...
void pbd::computeGradCGreen(Real restVolume, const Matrix3r &invRestMat, const Matrix3r &sigma, Vector3r *J)
{
	Matrix3r H;
	Matrix3r T;
...
void pbd::computeGreenStrainAndPiolaStressInversion(
	const Vector3r &x1, const Vector3r &x2, const Vector3r &x3, const Vector3r &x4,
	const Matrix3r &invRestMat,
	const Real restVolume,
	const Real mu, const Real lambda, Matrix3r &epsilon, Matrix3r &sigma, Real &energy)
{
	...
	Matrix3r U, VT;
	Vector3r hatF;
	decomposition::svdWithInversionHandling(F, hatF, U, VT);
	...
	sigmaDiag.setZero();
	sigmaDiag(0, 0) = sigmaVec[0];
	sigmaDiag(1, 1) = sigmaVec[1];
	sigmaDiag(2, 2) = sigmaVec[2];

	epsDiag.setZero();
	epsDiag(0, 0) = epsilonHatF[0];
	epsDiag(1, 1) = epsilonHatF[1];
	epsDiag(2, 2) = epsilonHatF[2];
...
bool pbd::solve_FEMTetraConstraint(
	const Vector3r &p0, Real invMass0, 
	const Vector3r &p1, Real invMass1,
	const Vector3r &p2, Real invMass2,
	const Vector3r &p3, Real invMass3,
...
bool pbd::init_ParticleTetContactConstraint(
...
bool pbd::solve_ParticleTetContactConstraint(
	const Real invMass0,							// inverse mass is zero if particle is static
	const Vector3r &x0,								// particle which collides with tet
	const Real invMass[],							// inverse masses of tet particles
	const Vector3r x[],								// positions of tet particles
	const Vector3r &bary,							// barycentric coordinates of contact point in tet
	Matrix3r &constraintInfo,		// precomputed contact info
	Real &lambda,
	Vector3r &corr0,
	Vector3r corr[])
{
...
bool pbd::velocitySolve_ParticleTetContactConstraint(
	const Real invMass0,							// inverse mass is zero if particle is static
	const Vector3r &x0,								// particle which collides with tet
	const Vector3r &v0,								// velocity of particle
	const Real invMass[],							// inverse masses of tet particles
