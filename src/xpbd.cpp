	...
	Vector3r& corr0, Vector3r& corr1, Vector3r& corr2, Vector3r& corr3)
{
	...
	Real mu_ = static_cast<Real>(0.5) / (static_cast<Real>(1.0) + poissonRatio);
	Real lambda_ = poissonRatio / ((static_cast<Real>(1.0) + poissonRatio) * (static_cast<Real>(1.0) - static_cast<Real>(2.0) * poissonRatio));
	...
	{
		pbd::computeGreenStrainAndPiolaStress(p0, p1, p2, p3, invRestMat, restVolume, mu_, lambda_, epsilon, sigma, U_);
		pbd::computeGradCGreen(restVolume, invRestMat, sigma, gradU_);
	}
	else
	{
		pbd::computeGreenStrainAndPiolaStressInversion(p0, p1, p2, p3, invRestMat, restVolume, mu_, lambda_, epsilon, sigma, U_);
		pbd::computeGradCGreen(restVolume, invRestMat, sigma, gradU_);
	}
	...
	const Real C = sqrt(static_cast<Real>(2.0) * U_);
