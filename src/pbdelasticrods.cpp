...
bool pbdelasticrods::solve_PerpendiculaBisectorConstraint(
...
bool pbdelasticrods::solve_GhostPointEdgeDistanceConstraint(
	const Vector3r& p0, Real invMass0,
	const Vector3r& p1, Real invMass1,
	const Vector3r& p2, Real invMass2,
	const Real stiffness,
...
bool pbdelasticrods::solve_DarbouxVectorConstraint(
	const Vector3r& p0, Real invMass0,
	const Vector3r& p1, Real invMass1,
	const Vector3r& p2, Real invMass2,
	...
	Vector3r& corr0, Vector3r& corr1, Vector3r& corr2, Vector3r& corr3, Vector3r& corr4)
{
	...
	pbdelasticrods::computeMaterialFrame(p0, p1, p3, d0);
	pbdelasticrods::computeMaterialFrame(p1, p2, p4, d1);

	pbdelasticrods::computeDarbouxVector(d0, d1, midEdgeLength, darboux_vector);
...
bool pbdelasticrods::computeMaterialFrame(
	const Vector3r& p0,
	const Vector3r& p1,
	const Vector3r& p2,
	Matrix3r& frame)
...
bool pbdelasticrods::computeDarbouxVector(const Matrix3r& dA, const Matrix3r& dB, const Real mid_edge_length, Vector3r& darboux_vector)
{
...
bool pbdelasticrods::computeMaterialFrameDerivative(
	...
	Matrix3r& d3p0, Matrix3r& d3p1, Matrix3r& d3p2)
{
	...
	matrix::crossProductMatrix(p2 - p1, product_matrix);
	d2p0 = mat * product_matrix;

	matrix::crossProductMatrix(p0 - p2, product_matrix);
	d2p1 = mat * product_matrix;

	matrix::crossProductMatrix(p1 - p0, product_matrix);
	...
	matrix::crossProductMatrix(d.col(2), product_mat_d3);
	matrix::crossProductMatrix(d.col(1), product_mat_d2);
...
bool pbdelasticrods::computeDarbouxGradient(
