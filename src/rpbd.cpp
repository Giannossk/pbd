...
bool rpbd::solve_BallJoint(
	...
	const Quaternionr &q1,	
	const Matrix<Real, 3, 4, DontAlign> &ballJointInfo,
	...
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	...
	// Compute Kinv
	Matrix3r K1, K2;
	matrix::computeMatrixK(connector0, invMass0, x0, inertiaInverseW0, K1);
	matrix::computeMatrixK(connector1, invMass1, x1, inertiaInverseW1, K2);
...
bool rpbd::update_DistanceJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_DistanceJoint(
	...
	const Real dt,
	const Matrix<Real, 3, 4, DontAlign> &jointInfo,
	Real &lambda,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	...
	// compute K = J M^-1 J^T
	Matrix3r K1, K2;
	matrix::computeMatrixK(c0, invMass0, x0, inertiaInverseW0, K1);
	matrix::computeMatrixK(c1, invMass1, x1, inertiaInverseW1, K2);
	Vector3r dir = c0 - c1;
...
bool rpbd::init_BallOnLineJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::update_BallOnLineJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_BallOnLineJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	...
	const Matrix3r &inertiaInverseW1,
	const Quaternionr &q1,
	const Matrix<Real, 3, 10, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	// jointInfo contains
	...
	// projection 
	const Matrix<Real, 3, 2> PT = jointInfo.block<3, 2>(0, 8);
	const Matrix<Real, 2, 3> P = PT.transpose();
	...
	)
{
	...
	Matrix<Real, 4, 4, DontAlign> Qq00, QHatq10;
	matrix::computeMatrixQ(q00, Qq00);
	matrix::computeMatrixQHat(q10, QHatq10);
	Matrix<Real, 2, 4, DontAlign> Pr = (QHatq10.transpose() * Qq00).block<2, 4>(2, 0);
	jointInfo.block<4, 2>(0, 0) = Pr.transpose();

	return true;
}

bool rpbd::update_HingeJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_HingeJoint(
	...
	const Matrix3r &inertiaInverseW1,
	const Quaternionr &q1,
	const Matrix<Real, 4, 7, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	// jointInfo contains
	...
	const Matrix<Real, 2, 4> &Pr = jointInfo.block<4, 2>(0, 0).transpose();
	...
	matrix::crossProductMatrix(r0, r0_star);
	matrix::crossProductMatrix(r1, r1_star);

	Matrix<Real, 4, 3, DontAlign> Gq1;
	matrix::computeMatrixG(q1, Gq1);
	Matrix<Real, 4, 4, DontAlign> Qq0;
	matrix::computeMatrixQ(q0, Qq0);

	const Matrix<Real, 2, 3> t = -Pr * (Qq0.transpose() * Gq1);
	...
	if (invMass0 != 0.0)
	{
		...
		Matrix3r K00;
		matrix::computeMatrixK(c0, invMass0, x0, inertiaInverseW0, K00);
	...
	if (invMass1 != 0.0)
	{
		...
		Matrix3r K11;
		matrix::computeMatrixK(c1, invMass1, x1, inertiaInverseW1, K11);

		K.block<3, 3>(0, 0) += K11;
		Matrix<Real, 3, 2> K03 = -r1_star * inertiaInverseW1 * t.transpose();
		K.block<3, 2>(0, 3) += K03;
		K.block<2, 3>(3, 0) += K03.transpose();
		K.block<2, 2>(3, 3) += t * inertiaInverseW1 * t.transpose();
	}

	const Matrix<Real, 5, 1> lambda = K.llt().solve(-C);
...
bool rpbd::init_UniversalJoint(
	...
	Matrix<Real, 3, 8, DontAlign> &jointInfo
	)
{
	// jointInfo contains
	// 0:	connector in body 0 (local)
...
bool rpbd::update_UniversalJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_UniversalJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	...
	const Matrix3r &inertiaInverseW1,
	const Quaternionr &q1,
	const Matrix<Real, 3, 8, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	// jointInfo contains
	...
	if (invMass0 != 0.0)
	{
		...
		K(3, 3) = (u.transpose() * inertiaInverseW0 * u)(0, 0);
	}
	if (invMass1 != 0.0)
	{
		// Jacobian for body 1 is
		...
		K(3, 3) += (u.transpose() * inertiaInverseW1 * u)(0, 0);
	}

	const Matrix<Real, 4, 1> lambda = K.llt().solve(b);
	...
	)
{
	...
	const Matrix< Real, 2, 3 > P = R0.block<3, 2>(0, 1).transpose();
	...
	matrix::computeMatrixQ(q00, Qq00);
	matrix::computeMatrixQHat(q10, QHatq10);
	Matrix<Real, 3, 4> Pr = (QHatq10.transpose() * Qq00).block<3, 4>(1, 0);
	jointInfo.block<4, 3>(0, 3) = Pr.transpose();

	return true;
}

bool rpbd::update_SliderJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_SliderJoint(
	...
	const Matrix3r &inertiaInverseW1,
	const Quaternionr &q1,
	const Matrix<Real, 4, 6, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	// jointInfo contains
	...
	const Matrix<Real, 3, 4> &Pr = jointInfo.block<4, 3>(0, 3).transpose();
...
bool rpbd::init_TargetPositionMotorSliderJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::update_TargetPositionMotorSliderJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_TargetPositionMotorSliderJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	...
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	...
	const Matrix<Real, 6, 1> lambda = K.llt().solve(-C);
...
bool rpbd::init_TargetVelocityMotorSliderJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::update_TargetVelocityMotorSliderJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_TargetVelocityMotorSliderJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
...
bool rpbd::velocitySolve_TargetVelocityMotorSliderJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Vector3r &v0,
	const Matrix3r &inertiaInverseW0,
...
bool rpbd::init_TargetAngleMotorHingeJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
	const Vector3r &position,
	const Vector3r &direction,
	Matrix<Real, 4, 8, DontAlign> &jointInfo
	)
{
	// jointInfo contains
...
bool rpbd::update_TargetAngleMotorHingeJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_TargetAngleMotorHingeJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	...
	const Matrix<Real, 4, 8, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	// jointInfo contains
	...
	const Matrix<Real, 3, 4> &Pr = jointInfo.block<4, 3>(0, 0).transpose();
	...
	if (invMass1 != 0.0)
	{
		...
		K.block<3, 3>(0, 0) += K11;
		Matrix<Real, 3, 3> K03 = -r1_star * inertiaInverseW1 * t.transpose();
		K.block<3, 3>(0, 3) += K03;
	...
	auto& llt = K.llt();
	const Matrix<Real, 6, 1> lambda = llt.solve(-C);
	if (llt.info() != Success)
	{
...
bool rpbd::init_TargetVelocityMotorHingeJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
	...
	)
{
	...
	matrix::computeMatrixQ(q00, Qq00);
	matrix::computeMatrixQHat(q10, QHatq10);
	Matrix<Real, 2, 4> Pr = (QHatq10.transpose() * Qq00).block<2, 4>(2, 0);
	jointInfo.block<4, 2>(0, 0) = Pr.transpose();

	return true;
}

bool rpbd::update_TargetVelocityMotorHingeJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	const Quaternionr &q1,
...
bool rpbd::solve_TargetVelocityMotorHingeJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	...
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	Matrix<Real, 4, 7> hingeJointInfo = jointInfo.block<4, 7>(0, 0);
...
bool rpbd::velocitySolve_TargetVelocityMotorHingeJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Vector3r &v0,
	const Matrix3r &inertiaInverseW0,	
	...
	Vector3r &corr_x1, Quaternionr &corr_q1)
{
	...
	C(0, 0) += alpha * lambda;
	const Matrix<Real, 6, 1> delta_lambda = K.llt().solve(-C);
...
bool rpbd::init_RigidBodyParticleBallJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	Matrix<Real, 3, 2, DontAlign> &jointInfo
	)
{
	// jointInfo contains
...
bool rpbd::update_RigidBodyParticleBallJoint(
	const Vector3r &x0,
	const Quaternionr &q0,
	const Vector3r &x1,
	Matrix<Real, 3, 2, DontAlign> &jointInfo
...
bool rpbd::solve_RigidBodyParticleBallJoint(
	const Real invMass0,
	const Vector3r &x0,
	const Matrix3r &inertiaInverseW0,
	const Quaternionr &q0,
	const Real invMass1,
	const Vector3r &x1,
	const Matrix<Real, 3, 2, DontAlign> &jointInfo,
	Vector3r &corr_x0, Quaternionr &corr_q0,
	Vector3r &corr_x1)
{
	// jointInfo contains
...
bool rpbd::init_RigidBodyContactConstraint(
	...
	Matrix<Real, 3, 5, DontAlign> &constraintInfo)
{
	...
	// determine K matrix
	Matrix3r K1, K2;
	matrix::computeMatrixK(cp0, invMass0, x0, inertiaInverseW0, K1);
	matrix::computeMatrixK(cp1, invMass1, x1, inertiaInverseW1, K2);
	Matrix3r K = K1 + K2;
...
bool rpbd::velocitySolve_RigidBodyContactConstraint(
	const Real invMass0,							// inverse mass is zero if body is static
	const Vector3r &x0, 						// center of mass of body 0
	const Vector3r &v0,						// velocity of body 0
	const Matrix3r &inertiaInverseW0,		// inverse inertia tensor (world space) of body 0
	...
	Matrix<Real, 3, 5, DontAlign> &constraintInfo,		// precomputed contact info
	Vector3r &corr_v0, Vector3r &corr_omega0,
	Vector3r &corr_v1, Vector3r &corr_omega1)
{
	// constraintInfo contains
...
bool rpbd::init_ParticleRigidBodyContactConstraint(
	const Real invMass0,							// inverse mass is zero if body is static
	const Vector3r &x0,						// center of mass of body 0
	const Vector3r &v0,						// velocity of body 0
	const Real invMass1,							// inverse mass is zero if body is static
	...
	Matrix<Real, 3, 5, DontAlign> &constraintInfo)
{
	...
	// determine K matrix
	Matrix3r K;
	matrix::computeMatrixK(cp1, invMass1, x1, inertiaInverseW1, K);
	if (invMass0 != 0.0)
	{
		K(0, 0) += invMass0;
		K(1, 1) += invMass0;
...
bool rpbd::velocitySolve_ParticleRigidBodyContactConstraint(
	const Real invMass0,							// inverse mass is zero if body is static
	const Vector3r &x0, 						// center of mass of body 0
	const Vector3r &v0,						// velocity of body 0
	const Real invMass1,							// inverse mass is zero if body is static
